...
function TUDflame(;L=2^5,Re=1_000,U=1,mem=Array,T=Float32)
    ...
    points = SMatrix{size(pnts)...}(pnts.*2L.+SA[2L,2L])
    # make the body
...
t₀,duration,tstep = round(sim_time(sim)), 10.0, 0.1
